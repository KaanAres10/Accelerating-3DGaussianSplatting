...
__device__ void computeColorFromSH(
    ...
    const __half* shs,            // Input -> __half*
    const bool* clamped, 
    const glm::vec3* dL_dcolor, 
    glm::vec3* dL_dmeans, 
    __half* dL_dshs)              // Output Gradient -> __half*
{
	...
	// Read SH coefficients from global memory (FP16)
    const __half* sh_ptr_in = shs + idx * max_coeffs * 3;
	glm::vec3 sh_local[16]; // Local buffer to store FP32 SH coeffs
    for (int i = 0; i < max_coeffs; ++i) {
        sh_local[i].x = __half2float(sh_ptr_in[i * 3 + 0]);
        sh_local[i].y = __half2float(sh_ptr_in[i * 3 + 1]);
        sh_local[i].z = __half2float(sh_ptr_in[i * 3 + 2]);
    }
	glm::vec3* sh = sh_local; // Let sh point to local FP32 buffer
	...
	// Prepare local FP32 buffer to store output gradients
	glm::vec3 dL_dsh_local[16]; // Local buffer to store FP32 SH gradients
	glm::vec3* dL_dsh = dL_dsh_local; // Write gradients to local FP32 buffer
	...
	// Write back SH gradients to global memory (FP16)
	__half* dL_dsh_out = dL_dshs + idx * max_coeffs * 3;
    for(int i = 0; i < max_coeffs; ++i) {
        dL_dsh_out[i * 3 + 0] = __float2half(dL_dsh_local[i].x);
        dL_dsh_out[i * 3 + 1] = __float2half(dL_dsh_local[i].y);
        dL_dsh_out[i * 3 + 2] = __float2half(dL_dsh_local[i].z);
    }
	...
	bool antialiasing)
{
	...
	preprocessCUDA<NUM_CHANNELS> << < (P + 255) / 256, 256 >> > (
		...
		radii,
		//shs,
		(const __half*)shs,  // cast to half pointer
		clamped,
		(glm::vec3*)scales,
		(glm::vec4*)rotations,
		scale_modifier,
		...
		dL_dcov3D,
		//dL_dsh,
		(__half*)dL_dsh, // cast to half pointer
		dL_dscale,
		dL_drot,
		dL_dopacity);
}
